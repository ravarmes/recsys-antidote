...
def read_movielens_1M(n_movies, n_users, top_users, data_dir='Data/MovieLens-1M'):
    ...
    # selecione os top n_movies com maior número de avaliações
    num_ratings = (~ratings.isnull()).sum(axis=1) # quantitativo de avaliações para cada filme: Toy Story (1995): 2077, Jumanji (1995): 701, etc...
    rows = num_ratings.nlargest(n_movies) # quantitativo de avaliações para cada filme (n_movies) ordenados: American Beauty (1999): 3428, Star Wars: Episode IV (1977): 2991
    ratings = ratings.loc[rows.index] # matriz com [500 rows x 6040 columns]; antes eram [3706 rows x 6040 columns]
    
    if top_users:
        # selecione os top n_users com o maior número de avaliações
        num_ratings = (~ratings.isnull()).sum(axis=0) # quantitativo de avaliações feitas por cada usuário [usuário, quantidade]
        cols = num_ratings.nlargest(n_users) # quantitativo de avaliações por cada usuário (n_users) ordenados
        ratings = ratings[cols.index] # matriz com [500 rows x 300 columns]; antes eram [500 rows x 6040 columns]
    else:
        # selecione os primeiros n_users da matriz
        cols = ratings.columns[0:n_users] # quantitativo de avaliações por cada um dos n_users
        ratings = ratings[cols] # matriz com [500 rows x 300 columns]; antes eram [500 rows x 6040 columns]

    ratings = ratings.T # transposta
